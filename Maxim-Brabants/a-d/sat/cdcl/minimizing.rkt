 ...
 (define (cdcl formula)
                                  ...
                                  (let ((other-lit (twls:other-watch watched-cls not-l)))
                                    ...
                                    (cond ((true? interpret other-lit)
                                          ...
                                          ((unknown? interpret other-lit) ; potential unit
                                           ...
                                           ;(set-propagated! other-lit interpret)  memorize propagated variable
                                           (set! antecedents (cons (cons other-lit 'prop) antecedents))
                                           (set-antecedents! other-lit antecedents interpret))
   ...
   (define (minimize-1-uip-clause confl-cls)
     ...
     (let* ((size (clause-size confl-cls))
           (remaining-literals
            (remp (lambda (lit)
                    (let ((antecedents (get-lit-antecedents lit interpret)))
                      ;; if literal was propagated and thus has antecedents
                      (display "----------------->")(display (variable lit))(newline)
                      (if antecedents
                          (let loop
                            ((lst (cdr antecedents)))
                            (cond ((marked? (caar lst) interpret)
                                   #f
                                   (set! size (- size 1)))
                                  ((eq? (cdar lst) 'dec)
                                   #t)
                                  (else (loop (cdr lst)))))
                          #t))) ;; can't be deleted from the clause
                  (literals confl-cls))))
