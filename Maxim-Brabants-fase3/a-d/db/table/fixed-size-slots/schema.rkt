...
(library
 ...
 (export new open delete!
         ...
         fixed-header-size disk position atts atts! remove-attribute!)
 ...
 (define-record-type schema
   ...
   schema?
   (b block)
   (a atts atts!))   ;; we also store the Scheme representation
 ...
 ; removes type + size from attribute index
 (define (delete-att! scma indx)
   (let loop
     ((idx 0))
     (if (= idx indx)
         (atts! scma (removeFromList (atts scma) idx))
         (loop (+ idx 1)))))
 

 ;; modify the schema block
 (define (remove-attribute! scma indx)
   (define rsz (- (record-size scma) (size scma indx)))
   (define cap (div (* (- disk:block-size fixed-header-size) 8) 
                    (+ (* rsz 8) 1)))
   (define nr-attributes (- (nr-of-attributes scma) 1))
   (capacity!         scma cap)
