...
(library
 ...
 (export new open close! current current!
         drop! insert! replace! delete! peek print
         set-current-to-first! set-current-to-next!
         old-scma old-scma! schema name disk)
 ...
 (define (peek scma tble)              ;; pass a schema for reading a tuple
   (define bffr (buffer tble))
   (define curr (slot tble))
   (if (null? bffr)
       no-current
       (node:record scma bffr curr)))
...
;; ========================================================
 ...
 ;; replace the tuple on rcid by 'tupl'
...
;; ========================================================
 ...
 (define (set-current-to-first! scma tble)                  ;; pass a schema for setting current
  ...
  (define (set-current-to-next! scma tble)                       ;; pass a schema for setting current
   (define bffr (buffer tble))
   (define curr (slot   tble))
   (if (null? bffr)
       no-current
